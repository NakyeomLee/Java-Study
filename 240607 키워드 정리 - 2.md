...
- 객체가 가진 값에 따라 동일한 기능 안에서 출력값도 바뀜
...
#### 정수 번호에 해당하는 문자 출력
System.out.println(line.charAt(index)); 활용용
...
public class TestString4 {
	public static void main(String[] args) {
		...
		// index (0부터 시작 해서 1씩 증가되는 정수 번호 (길이 - 1))
...
// 사용자에게 한 줄의 문자열을 입력받고 'o' 문자가 몇 번째에 있는 지 알려주는 프로그램
...
public class TestString7 {
	public static void main(String[] args) {
		...
		for (int i = 0; i < line.length(); i++) {
			...
			// 문자행 입력할땐 작은따옴표
			if (ch == 'o') {
				System.out.println(i + "번째에 'o'가 있습니다.");
				=> oracle 입력할 경우
				   0번째에 'o'가 있습니다. 출력
			}
		}
